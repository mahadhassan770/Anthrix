...
export default function ExecutiveDashboardClient({ data }: { data: DashboardDataProps }) {
  ...
  return (
    <div className="space-y-8 w-full max-w-7xl mx-auto">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 pb-6 border-b border-white/5">
        <div>
          <div className="flex items-center gap-2.5 mb-1.5">
            ...
            <span
              className={`text-[10px] font-mono uppercase px-2.5 py-0.5 rounded-full font-semibold tracking-wider ${
                data.isSuperAdmin
        ...
        <div className="flex flex-wrap items-center gap-3">
          ...
          <Link
            ...
            className="flex items-center gap-1.5 px-3.5 py-2 rounded-xl text-xs font-semibold text-white bg-white/[0.04] hover:bg-white/[0.08] border border-white/10 transition-all"
          >
          ...
          <Link
            href="/admin/revenue"
            className="flex items-center gap-1.5 px-3.5 py-2 rounded-xl text-xs font-semibold text-white bg-gradient-to-r from-[#F55036] to-[#D93520] hover:scale-[1.02] active:scale-[0.98] transition-all shadow-[0_2px_12px_rgba(245,80,54,0.3)]"
          >
            <DollarSign size={14} />
      ...
      {/* ══════════════════════════════════════════
          2. CORE KPI RIBBON (4 Clean Uniform Cards)
      ══════════════════════════════════════════ */}
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        {/* Card 1: Verified Revenue */}
        <Link
          href="/admin/revenue"
          className="group bg-[#080B12] hover:bg-white/[0.02] border border-white/10 hover:border-[#F55036]/40 rounded-2xl p-5 transition-all duration-200 flex flex-col justify-between"
        >
          ...
          <div>
            <p className="text-2xl font-bold text-white font-[family-name:var(--font-orbitron)] leading-none mb-1">
              {symbol}
            ...
            <p className="text-[11px] font-mono text-emerald-400 flex items-center gap-1">
              <TrendingUp size={11} /> Settled collections
            </p>
          </div>
        </Link>
        ...
        <Link
          href="/admin/clients"
          className="group bg-[#080B12] hover:bg-white/[0.02] border border-white/10 hover:border-[#38BDF8]/40 rounded-2xl p-5 transition-all duration-200 flex flex-col justify-between"
        >
          <div className="flex items-center justify-between mb-3">
            <span className="text-xs font-mono uppercase tracking-wider text-white/40">
              Active Clients
          ...
          <div>
            ...
            <p className="text-[11px] font-mono text-white/40">Registered accounts</p>
          </div>
        </Link>
        ...
        <Link
          href="/admin/projects"
          className="group bg-[#080B12] hover:bg-white/[0.02] border border-white/10 hover:border-violet-500/40 rounded-2xl p-5 transition-all duration-200 flex flex-col justify-between"
        >
          <div className="flex items-center justify-between mb-3">
            <span className="text-xs font-mono uppercase tracking-wider text-white/40">
              Portfolio Projects
          ...
          <div>
            ...
            <p className="text-[11px] font-mono text-white/40">Case studies &amp; systems</p>
          </div>
        </Link>
          ...
          <div>
            <div className="flex items-baseline gap-2">
              ...
              {data.unreadMessages > 0 && (
                <span className="text-[10px] font-mono bg-[#F55036] text-white px-1.5 py-0.5 rounded-full font-bold">
      ...
      {/* ══════════════════════════════════════════
          3. MAIN ORGANIZED TWO-COLUMN WORKSPACE
      ══════════════════════════════════════════ */}
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">

        {/* ── LEFT COLUMN (2/3 width): Telemetry & Pipeline ── */}
        <div className="lg:col-span-2 space-y-6">

          {/* Tabbed Telemetry Hub (Revenue Chart OR Ingestion Velocity) */}
          ...
          {/* Practice Area & Technical Stack Load */}
          <CapabilitiesMatrix data={data.practiceDistribution} />
        </div>

        {/* ── RIGHT COLUMN (1/3 width): Operations & Activity ── */}
        <div className="space-y-6">
          ...
          <div className="bg-[#080B12] border border-white/10 rounded-2xl p-5">
            <div className="flex items-center justify-between mb-4 pb-3 border-b border-white/5">
              <div className="flex items-center gap-2">
                <MessageSquare size={14} className="text-[#F55036]" />
                <h3 className="text-sm font-semibold text-white">Recent Inquiries</h3>
              </div>
              <Link
                href="/admin/messages"
                className="text-xs font-mono text-white/40 hover:text-[#F55036] transition-colors flex items-center gap-1"
              >
            ...
            {data.recentMessages.length === 0 ? (
              <p className="text-xs font-mono text-white/30 py-4 text-center">No messages yet</p>
            ) : (
                ...
                {data.recentMessages.map((msg) => (
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      <p className="text-xs font-semibold text-white group-hover:text-[#F55036] truncate transition-colors">
                        {msg.name}
                      </p>
          ...
          <div className="bg-[#080B12] border border-white/10 rounded-2xl p-5">
            ...
            {data.recentProjects.length === 0 ? (
              <p className="text-xs font-mono text-white/30 py-4 text-center">No projects yet</p>
            ) : (
              <div className="space-y-3">
                {data.recentProjects.map((p) => (
                  ...
                  >
                    <div className="min-w-0 flex-1">
                      <p className="text-xs font-semibold text-white group-hover:text-violet-300 truncate transition-colors">
                        {p.title}
                      </p>
